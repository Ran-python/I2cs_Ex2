...
/**
 * Intro2CS_2026A
 * A simple, self contained GUI for editing and visualizing Map2D objects with StdDraw.
 * Features:
 *  - draw points, rectangles, segments and circles
 *  - flood fill and shortest-path visualization
 *  - color palette and map size presets
 *  - optional load/save of maps from text files (first line: w h, then rows of ints)
 */
public class Ex2_GUI {
    // ---------- constants ----------
    private static final double HUD_HEIGHT = 2.4;           // area above the grid
    private static final double BUTTON_WIDTH = 3.4;
    private static final double BUTTON_HEIGHT = 1.1;
    private static final double BUTTON_MARGIN = 0.6;
    private static final double DROPDOWN_ROW = 1.0;
    ...
    /**
     * Draw/Run the GUI on a given map. If map is null, creates a default map.
     */
    ...
    private static void drawCells() {
        ...
        StdDraw.setPenColor(new Color(0, 0, 0, 35));
    ...
    private static double drawButton(double xLeft, double centerY, String title, boolean active) {
        ...
        StdDraw.setPenColor(active ? new Color(200, 200, 200) : new Color(215, 215, 215));
    ...
    private static void drawDropdown(Panel panel, int mapH) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            StdDraw.setPenColor(Color.BLACK);
            StdDraw.textLeft(left + 0.3, cy, items.get(i));
        }
    }
    ...
    private static void applyTool(Pixel2D p) {
        switch (tool) {
            ...
            case SEGMENT -> {
                ...
                else { canvas.drawLine(anchor, p, brush); anchor = null; }
            }
            case RECT -> {
                if (anchor == null) anchor = p;
                else { canvas.drawRect(anchor, p, brush); anchor = null; }
            }
            case CIRCLE -> {
                if (anchor == null) anchor = p;
                else {
            ...
            case FILL -> { canvas.fill(p, brush, cyclic); anchor = null; }
            case PATH -> {
                if (anchor == null) anchor = p;
                else {
